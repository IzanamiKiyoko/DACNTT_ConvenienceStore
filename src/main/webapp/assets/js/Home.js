...
{	
	try {
		...
		if (response.ok) 
		{
			let response2 = await fetch('/ConvenienceStore/DetailServlet?action=list');
			if (response2.ok) 
			{
			...
			<div class="card card-product">
				<div class="card-body">
				     <div class="text-center position-relative">
				         ...
				         <a href="#!" onclick="loadProductDetail(${id})"><img src="${img}" alt="Grocery Ecommerce Template" class="mb-3 img-fluid" /></div>
				                   </a>
								   <a href="#!" onclick="loadProductDetail(${id})">
...
function searchProduct(value="") {
	...
	if (!value)
	{
		search_value = document.getElementById("search-input").value.trim();
	}
	else
	{
		search_value = value;
    ...
    document.getElementById("SearchModalLabel").textContent = search_value;
    ...
    // Lấy container và xóa nội dung cũ
    const container = document.getElementById("productSearchContainer");
    container.innerHTML = '';

    if (filteredItems.length === 0) {
        // Hiển thị thông báo nếu không có sản phẩm nào
        container.innerHTML = '<p class="text-center text-muted">Không tìm thấy sản phẩm nào.</p>';
    } else {
        // Tạo nội dung HTML mới
        let htmlContent = '';
        filteredItems.forEach(item => {
            if (item.quantity === 0) {
                htmlContent += createProductCardHTML(item.id, item.name, item.price, item.image, '', 'Hết hàng', 'danger', 'disabled', 'secondary');
            } else if (item.price === 0) {
                htmlContent += createProductCardHTML(item.id, item.name, item.price, item.image, '', 'Miễn phí', 'success', '', 'primary');
            } else {
                htmlContent += createProductCardHTML(item.id, item.name, item.price, item.image, 'hidden', '', 'primary', '', 'primary');
            }
        });
        container.innerHTML = htmlContent; // Gán toàn bộ nội dung một lần
    }

    // Hiển thị modal
    const quickViewModal = bootstrap.Modal.getOrCreateInstance(document.getElementById('SearchModal'));
    quickViewModal.show();
}
function searchFromOther()
{
	window.location.href = "./index.html?action=search&text="+document.getElementById("search-input").value.trim();
}
