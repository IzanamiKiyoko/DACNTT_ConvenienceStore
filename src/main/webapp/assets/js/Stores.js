...
{	
	if (stores.length === 0)
	{
		try 
		{
			...
			if (response.ok) 
			{
				let response2 = await fetch('/ConvenienceStore/StoreLocationServlet?action=list');
				if (response2.ok) 
				{
					stores = await response2.json();
...
{
	return `
	<a href="#!" class="list-group-item d-flex flex-column align-items-start px-2 list-group-item-action" onclick="changeStore(${id},'${name}')" ${disabled}>
...
{
	const date = new Date();
	date.setTime(date.getTime() + (1*24*60*1000)); // Tính ngày hết hạn
